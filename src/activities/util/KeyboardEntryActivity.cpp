...
const char* KeyboardEntryActivity::germanTextForKey(const freeink::KeyEvent& ev) const {
  const bool shift = (ev.mods & 0x22) != 0;
  const bool altGr = (ev.mods & 0x40) != 0;
...
void KeyboardEntryActivity::ensureBleConnected() {
  if (inputType != InputType::Text) return;
  if (!powerLock) powerLock = makeUniqueNoThrow<HalPowerManager::Lock>();
